...
function renderSection(title, content) {
  return React.createElement("section", null,
    React.createElement("h2", { className: "text-2xl font-bold text-black mb-4 flex items-center gap-2" },
      title,
      React.createElement(ArrowRight, { className: "h-6 w-6 text-[#FF5C28]" })
    ),
    React.createElement("div", { className: "bg-[#FFB485] rounded-md p-6 text-white transition-all duration-300 hover:scale-[1.02] hover:shadow-lg" },
      content.map((item, index) => (
        typeof item === "string"
          ? React.createElement("p", { key: index }, item)
          : React.createElement("ul", { key: index, className: "list-disc list-inside space-y-2" },
            item.map((listItem, subIndex) =>
              React.createElement("li", { key: subIndex }, listItem)
            )
          )
      ))
    )
